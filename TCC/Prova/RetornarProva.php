<?php 
    ...
    if((isset($_GET['nivel_ensino']))&&(isset($_GET['topico']))&&(isset($_GET['tipo']))&&(isset($_GET['total']))) {
        ...
        if($tipo==0){
            ...
            $selecao .= "WHERE q.idtopico={$topico} AND q.nivel_ensino={$nivel_ensino} LIMIT {$total}";
            $produtos = mysqli_query($conecta,$selecao);
            if (!$produtos){
