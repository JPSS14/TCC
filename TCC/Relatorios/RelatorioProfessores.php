...
<?php
    ...
    $cx = $c->conexão();
    $p = new Professor();
    $pr = $p->totalProfessores($cx);
    $linha= mysqli_fetch_assoc($pr);
?>
            ...
            <div class="tela_alterar">
                <Div class="alterar_margen"> 
                    ...
                    <form action="RelatorioEstados.php" method="post" class="form-inline" >
                        ...
                        <label id="lblTopico" for="topico">Total de Professores: <?php echo (" ")?></label> 
                        <input style="margin-right: 1rem;" class="form-control" type="text" id="total" placeholder="usuario" value=" <?php echo $linha["total"];  ?>" readonly>
                     </form>
                </Div>
            </div> 
        </div> 
